...
export class DoublyLinkedList<T> implements Iterable<T> {
    private head: DoublyNode<T> | undefined = undefined;
    private tail: DoublyNode<T> | undefined = undefined;
    private length = 0;

    // Insert at the beginning (O(1))
    insertAtHead(value: T): void {
        const newNode: DoublyNode<T> = { value, next: this.head, prev: undefined };
        if (this.head) {
    ...
    insertAtTail(value: T): void {
        const newNode: DoublyNode<T> = { value, next: undefined, prev: this.tail };
        if (this.tail) {
    ...
    insertAt(index: number, value: T): void {
        ...
        const newNode: DoublyNode<T> = { value, next: curr, prev: curr!.prev };
    ...
    get(index: number): T | undefined {
        ...
        let curr: DoublyNode<T> | undefined;
