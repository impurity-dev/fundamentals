...
import { CircularDoubleLinkedList } from './circular-doubly.ts';
...
Deno.test('CircularDoubleLinkedList', async (t) => {
    await t.step('should initialize', () => {
        fc.assert(
            fc.property(safeArray, (items) => {
                const actual = new CircularDoubleLinkedList(items);
    ...
    await t.step('should insert at head', () => {
        fc.assert(
            fc.property(safeArray, safeArray, (inputs, initial) => {
                const actual = new CircularDoubleLinkedList(initial);
